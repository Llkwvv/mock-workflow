...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    # Updated paths for backend/frontend split
    EXEMPT_PATHS = (
        "/login", "/api/health", "/api/ws", "/output",
        "/", "/index.html", "/static"
    )
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if request.url.path.startswith("/api/") or request.url.path.startswith("/api/ws/"):
            raise HTTPException(status_code=401, detail="Unauthorized")

        # HTML page: redirect to login
        response = Response(status_code=307, headers={"Location": "/login.html"})
        return response
