...
class Settings(BaseSettings):
    ...
    llm_provider: str | None = Field(
        default=None,
        description="Preset provider: deepseek | modelscope | openai | ollama. When set, base_url and model are auto-filled unless explicitly overridden."
    )
    ...
    chroma_persist_dir: str = Field(
        default_factory=lambda: str(Path(__file__).resolve().parent.parent / "chroma_db"),
    ...
    rag_top_k_rules: int = Field(default=5, ge=1, le=20, description="Top-K rule retrieval for RAG")
    rag_top_k_samples: int = Field(default=3, ge=1, le=10, description="Top-K similar sample retrieval for RAG")
