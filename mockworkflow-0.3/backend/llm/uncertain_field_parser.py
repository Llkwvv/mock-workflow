...
def resolve_fields(
    ...
    if unresolved_columns and settings.llm_enabled:
        ...
        if model_to_use:
            try:
                ...
                import logging
                logging.getLogger(__name__).warning("LLM resolution failed (%s), falling back to minimal rules", exc)
    ...
    if settings.llm_enabled and settings.llm_value_pool_enabled:
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).warning("Value pool generation skipped (%s)", exc)
