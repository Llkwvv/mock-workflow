...
from backend.schemas.field import FieldSemantic, FieldSpec, SqlType
...
class RuleStore:
    ...
    def batch_merge(self, min_confidence: float = 0.3) -> dict[str, int]:
        """Clean up the rule store by removing low-confidence or duplicate rules.

        Returns:
            Stats dict with ``removed`` and ``merged`` counts.
        """
